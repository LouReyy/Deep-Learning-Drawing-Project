...
async function chargerModele() {
    const modeleSelectionne = modeleDropdown.value;
    console.log(`Modèle sélectionné : ${modeleSelectionne}`);
      await loadModel(modeleSelectionne);
  }
...
const loadModel = async (modeleSelectionne) => {
    console.log("Chargement du modèle...");

    console.log(`Modele selectionné : ${modeleSelectionne}`);

    const cheminComplet = `../${modeleSelectionne}.tflite`;

    console.log(`Chemin selectionné : ${cheminComplet}`);


    model = await tflite.loadTFLiteModel(cheminComplet);
    model.predict(tf.zeros([1, 28, 28, 1])); // warmup

    console.log(`Modèle chargé! (${LABELS.length} classes)`);
};
...
const drawPie = (top345) => {
    const probs = [];
    const labels = [];

    for (const pred of top345) {
...
const predict = async () => {
     ...
     console.log('Image Strokes:', imageStrokes);
...
function getRandomLabel() {
  ...
  const dessinLabel = document.getElementById('dessinLabel');
  
  dessinLabel.innerText = `Lets draw a : ${getRandomLabel()}`;
